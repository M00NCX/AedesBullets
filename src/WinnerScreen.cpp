...
WinnerScreen::WinnerScreen() : 
    background("./img/background_winner.jpg"), 
...
void WinnerScreen::handleMouse(int x, int y) {
...
void WinnerScreen::handleClick(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && hoverRestart) {
        Game::getInstance().setState(GameState::MENU);
    }
}

void WinnerScreen::renderTexture(Texture& texture, int x, int y, int width, int height, bool hover) {
